    ...
    return (
        <div className="w-full space-y-7 font-['Inter',ui-sans-serif,sans-serif]">
            ...
            <header className="border-b border-zinc-300 dark:border-zinc-700 pb-6">
                <p className="text-[10px] uppercase tracking-[.18em] text-zinc-600 dark:text-zinc-400">Project access</p>
                <h1 className="mt-3 font-['Instrument_Serif',Georgia,serif] text-5xl leading-none text-zinc-900 dark:text-white">Project members</h1>
                <p className="mt-3 text-sm text-zinc-600 dark:text-zinc-300">Assign delivery roles to people who already belong to the organisation.</p>
            </header>
            {error && <p className="border-y border-red-400/50 py-3 text-sm text-red-300">{error}</p>}
            <form className="grid gap-4 border border-zinc-300 dark:border-zinc-700 p-5 md:grid-cols-[minmax(0,1fr)_13rem_auto]" onSubmit={addMember}>
                <label className="text-[10px] uppercase tracking-[.14em] text-zinc-600 dark:text-zinc-300">
                    Add organisation member
                    <select className="mt-2 block w-full border border-zinc-400 dark:border-zinc-600 bg-white dark:bg-zinc-950 p-3 text-sm normal-case tracking-normal text-zinc-900 dark:text-white outline-none focus:border-zinc-900 dark:focus:border-white disabled:opacity-50" disabled={!availableMembers.length || busy} required value={selectedUserId} onChange={(event) => setSelectedUserId(event.target.value)}>
                ...
                <label className="text-[10px] uppercase tracking-[.14em] text-zinc-600 dark:text-zinc-300">
                    Project role
                    <select className="mt-2 block w-full border border-zinc-400 dark:border-zinc-600 bg-white dark:bg-zinc-950 p-3 text-sm normal-case tracking-normal text-zinc-900 dark:text-white outline-none focus:border-zinc-900 dark:focus:border-white" value={role} onChange={(event) => setRole(event.target.value as ProjectRole)}>
                        {roles.map((item) => <option key={item}>{item}</option>)}
                    </select>
                </label>
                <button className="inline-flex min-h-11 items-center justify-center gap-2 self-end border border-zinc-900 dark:border-white px-4 text-xs uppercase tracking-[.14em] text-zinc-900 dark:text-white disabled:opacity-50" disabled={busy || !availableMembers.length || !selectedUserId} type="submit">
            ...
            <section className="border border-zinc-300 dark:border-zinc-700">
                ...
                {members.length ? members.map((member) => (
                    <div className="grid grid-cols-[minmax(0,1fr)_13rem_44px] items-center gap-4 border-b border-zinc-200 dark:border-zinc-800 px-4 py-4 last:border-b-0" key={member.id}>
                        <div className="min-w-0">
                            <p className="truncate text-sm text-zinc-900 dark:text-white">{member.name || "Unnamed member"}</p>
                            <p className="mt-1 truncate text-xs text-zinc-600 dark:text-zinc-400">{member.email}</p>
                        </div>
                        <select className="border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 p-2 text-xs text-zinc-700 dark:text-zinc-300" value={member.role} onChange={(event) => void changeRole(member, event.target.value as ProjectRole)}>
                            {roles.map((item) => <option key={item}>{item}</option>)}
                        </select>
                        <button aria-label={`Remove ${member.name || member.email}`} className="text-zinc-600 dark:text-zinc-400 hover:text-red-300 disabled:opacity-50" disabled={removingId === member.id} onClick={() => void removeMember(member)} type="button">{removingId === member.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</button>
                    </div>
                )) : <p className="p-8 text-center text-sm text-zinc-600 dark:text-zinc-400">No project members yet.</p>}
            </section>
        </div>
    );
}
