...
export default function ProjectSettingsPage() {
    ...
    return (
        <div className="max-w-2xl space-y-8">
            ...
            <header><p className="text-xs uppercase tracking-[.18em] text-zinc-500">Project administration</p><h1 className="mt-3 text-4xl text-white">Project settings</h1></header>
            {error && <p className="border-y border-red-400/50 py-3 text-sm text-red-300">{error}</p>}
            <form className="space-y-5 border border-zinc-700 p-5" onSubmit={saveProject}>
                <div><label className="mb-2 block text-xs uppercase tracking-[.16em] text-zinc-400" htmlFor="project-name">Project name</label><input className="w-full border-b border-zinc-600 bg-transparent p-2 text-white focus:border-white focus:outline-none" id="project-name" maxLength={127} minLength={3} onChange={(event) => setName(event.target.value)} required value={name} /></div>
                <div><label className="mb-2 block text-xs uppercase tracking-[.16em] text-zinc-400" htmlFor="project-description">Description</label><textarea className="min-h-28 w-full border-b border-zinc-600 bg-transparent p-2 text-white focus:border-white focus:outline-none" id="project-description" maxLength={500} onChange={(event) => setDescription(event.target.value)} value={description} /></div>
                {validationMessage && <p className="text-sm text-red-300">{validationMessage}</p>}
                <button className="inline-flex min-h-10 items-center gap-2 border border-white px-4 text-xs uppercase tracking-[.14em] text-white disabled:cursor-not-allowed disabled:opacity-40" disabled={!hasChanges || Boolean(validationMessage) || saving} type="submit">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{saving ? "Saving" : "Save changes"}</button>
            </form>
            <section className="border border-red-500/40 p-5"><div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-red-300" /><div><h2 className="text-lg text-white">Delete project</h2><p className="mt-2 text-sm leading-6 text-zinc-400">This action is permanent. The API may also delete associated tickets.</p></div></div><button className="mt-5 inline-flex min-h-10 items-center gap-2 border border-red-400/60 px-4 text-xs uppercase tracking-[.14em] text-red-200 disabled:cursor-not-allowed disabled:opacity-40" disabled={deleting || !project} onClick={() => setDeleteModalOpen(true)} type="button">{deleting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}{deleting ? "Deleting" : "Delete project"}</button></section>
